<template>
  ...
  <div class="tableMain">
    ...
    <el-row>
      <el-col :span="4">
        <fieldset>
          <legend>输入用例id查询</legend>
          <el-input v-model="search_id" placeholder="请输入用例id"></el-input>
          <el-button type="primary" @click="search">查询并填充</el-button>
        </fieldset>
    ...
    <el-form :model="test_add" class="demo-ruleForm" ref="form" label-position="left" :rules="rules" size="small">
      ...
      <el-row>
        <el-col :span="3">
          <el-form-item>
            <el-button type="primary" @click="onSubmit">更新用例</el-button>
          </el-form-item>
        </el-col>
        <el-col :span="3">
          <el-form-item>
...
<script>
  export default {
    ...
    methods: {
      ...
      search() {
        this.$http.post(this.GLOBAL.BASE_URL + "get_test_by_id/", {
          "test_id": this.search_id
        }).then((response) => {
          if (response.data.retcode == 0) {
            this.$notify({
              title: '提示',
            ...
            this.test_add = response.data.data
          }
          if (response.data.retcode == -1) {
            this.$notify({
              title: '提示',
      ...
      onSubmit() {
        ...
        this.$http.post(this.GLOBAL.BASE_URL + "update_one_test/", this.test_add).then((response) => {
          this.$notify({
            title: '提示',
            message: response.data.msg,
            type: 'info'
